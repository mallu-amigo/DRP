async function recentReports() {
    let response = await fetch('../report_disaster/fetchReports.php');
    ...
    if (data.length > 0) {
        // Latest report
        if (data[0]) {
            document.getElementById('disaster_name_1').innerText = data[0].title;
            document.getElementById('disaster_location_1').innerHTML = `<i class="fa fa-location-dot"></i> ${data[0].location_input}`;
            document.getElementById('disaster_date_1').innerHTML = `<i class="fa fa-calendar"></i> ${new Date(data[0].datetime).toLocaleDateString()}`;
            document.getElementById('disaster_image_1').src = data[0].file;
        ...
        if (data[1]) {
            document.getElementById('disaster_name_2').innerText = data[1].title;
            document.getElementById('disaster_location_2').innerHTML = `<i class="fa fa-location-dot"></i> ${data[1].location_input}`;
            document.getElementById('disaster_date_2').innerHTML = `<i class="fa fa-calendar"></i> ${new Date(data[1].datetime).toLocaleDateString()}`;
            document.getElementById('disaster_image_2').src = data[1].file;
        }
    } else {
...
document.querySelectorAll('.view_report').forEach(button => {
